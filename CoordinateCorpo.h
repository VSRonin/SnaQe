...
public:
	...
	unsigned int Distanza(const CoordinateCorpo& a) const {
		return qAbs(x-a.x)+qAbs(y-a.y);
	}
	...
	CoordinateCorpo(const CoordinateCorpo& a) : x(a.x), y(a.y), direzione(a.direzione){}
	bool operator==(const CoordinateCorpo& a) const {return (x==a.x && y==a.y);}
	bool operator!=(const CoordinateCorpo& a) const {return !operator==(a);}
